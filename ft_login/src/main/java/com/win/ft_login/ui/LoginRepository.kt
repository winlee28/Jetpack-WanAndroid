...
class LoginRepository : BaseRepository() {
    ...
    private suspend fun requestLogin(username: String, password: String): NetResult<User> =
        executeResponse(
            RetrofitClient.getService(RequestCenter::class.java).login(username, password)
        )

    private suspend fun requestRegister(username: String, password: String,surePassword: String): NetResult<User> =
        executeResponse(
            RetrofitClient.getService(RequestCenter::class.java).register(username, password,surePassword)
        )
